...
class Resolution:
    ...
    def apply_resolution(self, initial_statement) -> bool:
        # print("Initial Statement: {}".format(initial_statement))
        ...
        if not initial_statement:
            return True
        for predicate, arguments_set in initial_statement.items():
                ...
                for statement in self.knowledge_base[negative_query[0]]:
                    for args in statement[negative_query[0]]:
                        ...
                        if unifiable:
                            ...
                            if self.apply_resolution(unified_statement):
                                return True
    ...
    def resolve_queries(self) -> str:
        ...
        for query in self.queries:
            negative_query = Resolution.get_negative_query(query)
            self.visited = set()
            result.append(self.apply_resolution({negative_query[0]: {negative_query[1]}}))
