...
class Resolution:
    ...
    def read_input2(self, input_file: str):
        """
    ...
    def str_hash2(statement):
        ...
        var_index = 0
        for predicate, arguments_set in sorted(statement.items()):
            for arguments in list(sorted(arguments_set)):
                arguments_copy = list(arguments)
                for index, argument in enumerate(arguments):
                    if argument.islower():
                        if argument not in var_assigns:
                            var_assigns[argument] = "var" + str(index)
                            index += 1
                        arguments_copy[index] = var_assigns[argument]

                result.append(predicate + str(tuple(arguments_copy)))
        return ' '.join(result)
    ...
    @staticmethod
    def perform_assignment(statement: dict, unifier: dict, elimination_literal: tuple = None, prev_statement=None):
        """
    ...
    def unify(statement1, statement2):
        ...
        while True:
                    ...
                    for arguments_set2 in statement2[predicate2]:
                        for arguments1 in arguments_set1:
                            for arguments2 in arguments_set2:
                                is_unifiable, unifier1, unifier2 = Resolution.is_unifiable2(arguments1, arguments2)
                                if is_unifiable:
            ...
            if unifier1 is not None:
                statement1 = Resolution.perform_assignment(statement1, unifier1, statement1_eliminate_literal)
                statement2 = Resolution.perform_assignment(statement2, unifier2, prev_statement=statement1)
            else:
                break
    ...
    def apply_resolution(self, initial_statement) -> bool:
        ...
        while stack:
            ...
            is_unifiable, unifier1, unifier2 = Resolution.is_unifiable2(initial_literal[1], kb_literal[1])
            if is_unifiable:
                assigned_initial_statement = Resolution.perform_assignment(initial_statement, unifier1,
                                                                           initial_literal)
                assigned_kb_statement = Resolution.perform_assignment(kb_statement, unifier2, kb_literal,
                                                                      assigned_initial_statement)
                ...
                #        "unifying_kb_literal = {}\nunifier1 = {}, unifier2 = {}\nresolvent_statement = {}\nvisited = {}\n")
                #       .format(dict(initial_statement), initial_literal, dict(kb_statement), kb_literal, unifier1,
                #               unifier2, dict(resolvent_statement), visited))
